  ...
  <div class="content" v-for="product in xinList" :key="product.id">
    ...
    <div class="new-user">
      ...
      <div class="new-user-number">
        ...
        <a href="javascript:void(0)" @click="goLink('/product/detail' , {pid : product.id })" class="new-user-btn">立即投资</a>
      </div>
      <span class="new-tag">新用户专享</span>
    </div>

    <h2 class="public-title"><span>优选产品</span> <a href="javascript:void(0)" @click="goLink('/product/more',{ptype : 1})"
    ...
    <h2 class="public-title"><span>散标产品</span> <a href="javascript:void(0)" @click="goLink('/product/more',{ptype : 2})"
                                                      class="public-title-more">查看更多产品>></a></h2>
    <ul class="preferred-select clearfix">
...
export default {
  ...
  mounted() {
    //发起ajax请求
    doGet('/app/info').then(resp => {
      if (resp.data.code === 1000) {
        this.appinfo = resp.data.info;
      }
    });
    doGet("/product/main").then(resp => {
      if (resp.data.code === 1000) {
        this.xinList = resp.data.info.xinList;
        this.youList = resp.data.info.youList;
        this.sanList = resp.data.info.sanList;
      }
    })
  },
  methods: {
